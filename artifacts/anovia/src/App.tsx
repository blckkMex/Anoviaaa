...
// Wave SVG divider — hot-pink fill, separates sections
...
function AnnouncementBar() {
  return (
    <div className="bg-[var(--color-navy)] text-center py-2 px-4 z-50 relative text-white text-xs sm:text-sm font-semibold tracking-wide">
      ...
      Free delivery on orders of Rs 599+ — Order on WhatsApp
...
function Header() {
  ...
  return (
    <header className={`sticky top-0 w-full z-40 transition-all duration-300 ${
      scrolled
        ? 'bg-white/95 backdrop-blur-md border-b border-[var(--color-bg-accent)] shadow-sm'
        : 'bg-transparent'
    }`}>
    ...
    <section id="promise" className="py-24 md:py-32 bg-white px-6">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16 md:mb-24">
          ...
          <FadeIn delay={0.1}>
            <h2 className="text-4xl md:text-5xl font-bold text-[var(--color-navy)] mb-6" style={{ fontFamily: 'Fredoka, sans-serif' }}>
              Small details, kept carefully
            </h2>
            <p className="text-lg text-[var(--color-navy-muted)] max-w-2xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {promises.map((p, i) => (
            <FadeIn key={i} delay={0.2 + i * 0.1}>
              <div className="bg-[var(--color-bg-light)] rounded-3xl p-10 text-center border-2 border-[var(--color-bg-accent)] hover:-translate-y-2 transition-all duration-500 shadow-sm hover:shadow-xl hover:shadow-[var(--color-bg-accent)] hover:border-[var(--color-pink)] h-full flex flex-col items-center">
                <div className="w-20 h-20 rounded-full bg-[var(--color-bg-accent)] flex items-center justify-center mb-6 shadow-sm">
                  {p.icon}
                </div>
                <h3 className="text-2xl font-bold text-[var(--color-navy)] mb-4" style={{ fontFamily: 'Fredoka, sans-serif' }}>
                  {p.title}
                </h3>
  ...
  return (
    <>
      ...
      <section id="shop" className="py-24 md:py-32 bg-[var(--color-navy)] px-6 relative overflow-hidden">
        {/* Decorative sparkles on dark bg */}
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="text-center mb-16 md:mb-20">
            <FadeIn>
              <span className="text-sm font-bold uppercase tracking-widest text-[var(--color-pink)] mb-4 block">
                Shop the Edit
              </span>
            </FadeIn>
            <FadeIn delay={0.1}>
            ...
            {categories.map((c, i) => (
              <FadeIn key={i} delay={0.1 * i} className="h-full">
                <a
                  ...
                  className="flex flex-col items-center text-center h-full border-2 border-white/20 rounded-3xl p-8 bg-white/10 backdrop-blur-sm hover:-translate-y-2 hover:bg-white/20 hover:border-[var(--color-pink)] transition-all duration-300 group"
                >
                  <div className="mb-6 transform group-hover:scale-110 transition-transform duration-500 text-white">
                    {c.glyph}
                  </div>
                  <h3 className="text-2xl font-bold text-white mb-2" style={{ fontFamily: 'Fredoka, sans-serif' }}>{c.name}</h3>
...
function MysteryScoop() {
  ...
  return (
    <section id="scoop" className="py-24 md:py-32 bg-[var(--color-bg-alt)] px-6 overflow-hidden relative">
      ...
      <SparkleIcon size={24} className="absolute top-1/4 left-8 text-[var(--color-navy)] opacity-30" />
      ...
      <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center gap-16 md:gap-24">
        <div className="flex-1 text-center md:text-left">
          ...
          <FadeIn delay={0.2}>
            <p className="text-lg md:text-xl text-[var(--color-navy-muted)] mb-10 max-w-md mx-auto md:mx-0">
          ...
          <FadeIn delay={0.3}>
            ...
            <p className="text-sm mt-4 text-[var(--color-navy-muted)] font-medium">
        ...
        <div className="flex-1 flex justify-center">
          <FadeIn delay={0.4} className="relative cursor-pointer" onClick={handleShake}>
            <div className={`relative ${isShaking ? 'animate-[shake_0.5s_cubic-bezier(.36,.07,.19,.97)_both]' : ''}`}>
              {/* Lid */}
              <div
                className={`w-40 h-8 bg-[var(--color-navy)] rounded-t-xl mx-auto relative z-20 transition-transform duration-200 ${isShaking ? '-translate-y-5 rotate-3' : ''} shadow-md`}
              >
              ...
              <div className="w-64 h-72 bg-white/60 backdrop-blur-md border-4 border-[var(--color-navy)] rounded-b-[40px] rounded-t-xl relative z-10 flex items-center justify-center p-8 shadow-2xl overflow-hidden">
                ...
                <div className="text-center z-30">
                  <span className="text-2xl md:text-3xl text-[var(--color-navy)] font-bold leading-tight" style={{ fontFamily: 'Fredoka, sans-serif' }}>
                    {currentItem}
                  </span>
                </div>
                {/* Decorative */}
      ...
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-16 md:gap-24">
        ...
        <div className="flex-1 order-2 md:order-1 flex justify-center w-full">
          <FadeIn className="w-full max-w-[400px] aspect-square relative">
            <div className="absolute inset-0 bg-[var(--color-bg-alt)] rounded-3xl overflow-hidden group hover:shadow-2xl hover:shadow-[var(--color-bg-accent)] transition-all duration-500 border-2 border-[var(--color-bg-accent)]">
              {/* Ribbon horizontal */}
        ...
        <div className="flex-1 order-1 md:order-2">
          ...
          <FadeIn delay={0.1}>
            <h2 className="text-4xl md:text-5xl font-bold text-[var(--color-navy)] mb-6 leading-tight" style={{ fontFamily: 'Fredoka, sans-serif' }}>
              The gift that does all the work for you
            </h2>
          </FadeIn>
          <FadeIn delay={0.2}>
            <p className="text-lg text-[var(--color-navy-muted)] mb-8">
          ...
          <FadeIn delay={0.3}>
            <div className="flex flex-wrap gap-3 mb-10">
              {tags.map((tag, i) => (
                <span
                  key={i}
                  className="px-4 py-2 bg-[var(--color-bg-alt)] text-[var(--color-navy)] rounded-full text-sm font-bold border-2 border-[var(--color-bg-accent)] hover:border-[var(--color-navy)] transition-colors"
                >
                  {tag}
                </span>
              ))}
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center justify-center gap-2 bg-[var(--color-navy)] text-white px-8 py-4 rounded-full font-bold hover:bg-[var(--color-navy-light)] transition-colors shadow-md hover:shadow-lg w-full sm:w-auto"
            >
    ...
    <section id="locations" className="py-24 bg-[var(--color-bg-alt)] px-6 relative overflow-hidden">
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {places.map((place, i) => (
            <FadeIn key={i} delay={0.2 + i * 0.1}>
              <div className="bg-white p-8 rounded-3xl border-2 border-[var(--color-bg-accent)] hover:-translate-y-2 transition-all duration-300 h-full flex flex-col shadow-sm hover:border-[var(--color-navy)] hover:shadow-lg">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <span className="text-xs font-bold uppercase tracking-wider text-[var(--color-navy-muted)] bg-[var(--color-bg-alt)] px-2 py-1 rounded-full">
                    {place.type}
                  </span>
                </div>
                <h3 className="text-2xl font-bold text-[var(--color-navy)] mb-3" style={{ fontFamily: 'Fredoka, sans-serif' }}>
                  {place.name}
                </h3>
    ...
    <section id="reviews" className="py-24 md:py-32 bg-white px-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {reviews.map((r, i) => (
            <FadeIn key={i} delay={0.2 + i * 0.1}>
              <div className="bg-[var(--color-bg-light)] p-8 rounded-3xl border-2 border-[var(--color-bg-accent)] h-full flex flex-col relative hover:border-[var(--color-pink)] hover:-translate-y-1 transition-all duration-300 hover:shadow-lg">
      ...
      <section className="bg-[var(--color-navy)] py-20 px-6 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10 flex flex-col md:flex-row items-center justify-between gap-10">
          <div className="text-center md:text-left">
            <h2 className="text-3xl md:text-5xl font-bold text-white mb-4" style={{ fontFamily: 'Fredoka, sans-serif' }}>
              Ready to order?
            </h2>
            <p className="text-white/80 text-lg font-medium">Send us a message to check availability.</p>
          </div>
          <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
            <a
              href="#shop"
              className="bg-white text-[var(--color-navy)] px-8 py-4 rounded-full font-bold hover:bg-[var(--color-bg-light)] transition-colors text-center shadow-md"
            >
              Shop the Edit
            </a>
            <a
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-12 mb-16 border-b border-white/10 pb-16">
        <div className="md:col-span-5">
          <a href="#" className="flex items-center gap-3 mb-6">
            <div className="w-10 h-10 rounded-full bg-[var(--color-pink)] flex items-center justify-center text-white font-bold text-xl shadow-md">
              A
            </div>
        ...
        <div className="md:col-span-3">
          ...
          <ul className="flex flex-col gap-4">
            {[
              ...
              { label: 'Mystery Scoop', href: '#scoop' },
              { label: 'Hampers & Corporate', href: '#hampers' },
              { label: 'Reviews', href: '#reviews' },
            ].map(({ label, href }) => (
              <li key={label}>
                <a href={href} className="text-white/60 hover:text-[var(--color-pink)] transition-colors font-medium">{label}</a>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div className="md:col-span-4">
          ...
          <ul className="flex flex-col gap-4">
            <li>
              <a href="https://instagram.com/anoviaaa.16" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-white/60 hover:text-[var(--color-pink)] transition-colors font-medium">
                <Instagram size={18} /> @anoviaaa.16
              </a>
            </li>
            <li>
              <a href="https://wa.me/message" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-white/60 hover:text-[var(--color-pink)] transition-colors font-medium">
                <MessageCircle size={18} /> WhatsApp Community
              </a>
            </li>
            <li>
              <a href="https://instagram.com/anoviaa.16" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-white/60 hover:text-[var(--color-pink)] transition-colors font-medium">
